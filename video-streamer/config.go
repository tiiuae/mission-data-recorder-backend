...
type ConfigST struct {
	...
	VideoServerAddress string              `json:"-"`
}
...
func (element *ConfigST) createURL(uuid string) string {
	if uuid == "test-stream-1" {
		return "rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mov"

	}
	if uuid == "test-stream-2" {
		return "rtsp://akaimp.duckdns.org/live/ch00_0"
	}

	return fmt.Sprintf("http://%s/%s", element.VideoServerAddress, uuid)
}
